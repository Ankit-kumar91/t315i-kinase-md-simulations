...
We ran molecular dynamics simulations on four BCR-ABL1 systems to see whether physics-based simulations can capture the structural basis of the T315I gatekeeper mutation's selective resistance: it disables imatinib, but ponatinib still works.
...
built by in-silico mutation, since no co-crystal exists, which is precisely the point.
...
hydrogen-bond counts, and binding-pocket contact occupancies. 
...
collapse. That's a prerequisite for interpreting the finer metrics.
...
overlap, meaning no mutation-induced destabilisation.*
...
- **CHARMM27** all-atom force field (via `pdb2gmx`) for protein topology.
- **SwissParam** (swissparam.ch) for ligand (imatinib/ponatinib) topology and CHARMM-compatible parameters generated from `LIG.mol2`; see [`gromacs_steps.md`](gromacs_steps.md) for the exact prep pipeline (mol2 bond-order fix via GROMACS' `sort_mol2_bonds.pl`, SwissParam upload, `gmx editconf` conversion to `.gro`).
- **ChimeraX**, **PyMOL** for structure inspection, complex-building sanity checks, and rendering
  of the representative snapshots in Figure 1 (`comparison/complex_structures.png`).
- **VMD** for trajectory visualization during setup/equilibration checks.
